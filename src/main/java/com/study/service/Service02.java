package com.study.service;

import com.study.mapper.Mapper06;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

@Service
@RequiredArgsConstructor
// service 내 모든 메소드에 적용됨
@Transactional(rollbackFor = Exception.class)
public class Service02 {
    private final Mapper06 mapper;

    public void transferMoney1() {
        // 강인이 돈 500원 뺏기
        mapper.minusMoney();
        // 흥민이 돈 500원 더하기
        mapper.plusMoney();
    }

    @Transactional
    public void transferMoney2() {
        // 강인이 돈 500원 뺏기
        mapper.minusMoney();
        if (true) {
            System.out.println("오류 발생!");
            throw new RuntimeException();
        }
        // 흥민이 돈 500원 더하기
        mapper.plusMoney();

    }

    // CheckedException 발생 시키기
    @Transactional
    public void transferMoney3() throws Exception {
        // 강인이 돈 500원 뺏기
        mapper.minusMoney();

        if (true) {
            System.out.println("오류 발생3!");
            throw new Exception();
        }
        // 흥민이 돈 500원 더하기
        mapper.plusMoney();
    }

    // 모든 예외 발생 시 롤백 (Exception.class = 런타임, check 모두 포함)
    @Transactional(rollbackFor = Exception.class)
    public void transferMoney4() throws Exception {
        // 강인이 돈 500원 뺏기
        mapper.minusMoney();

        if (true) {
            System.out.println("오류 발생3!");
            throw new Exception();
        }
        // 흥민이 돈 500원 더하기
        mapper.plusMoney();
    }

    @Transactional
    public void transferMoney5() {
        // 강인이 돈 500원 뺏기
        mapper.minusMoney();

        try {
            if (true) {
                System.out.println("오류 발생3!");
                throw new Exception();
            }
        } catch (Exception e) {
            throw new RuntimeException(e); // check exception 을 런타임 오류로 감싸서 던짐
        }
        // 흥민이 돈 500원 더하기
        mapper.plusMoney();

    }
}
